...
const formatDescription = (
  ...
  const lines: string[] = [
    ...
    "## Workflow",
    "",
    '1. `const matches = await tools.discover({ query: "<intent + key nouns>", limit: 12 });`',
    ...
    "5. Use `tools.executor.tools.search(...)` when you want ranked tool search explicitly, and `tools.executor.sources.list()` when you need configured source inventory.",
    "6. Call the tool: `const result = await tools.<path>(input);`",
    "",
    "## Rules",
    "",
    "- `tools.discover()` and `tools.executor.tools.search()` return ranked matches, best-first. Use short intent phrases like `github issues`, `repo details`, or `create calendar event`.",
    "- When you already know the namespace, narrow with `tools.discover({ namespace: \"github\", query: \"issues\" })` or `tools.executor.tools.search({ namespace: \"github\", query: \"issues\" })`.",
    "- Use `tools.executor.sources.list()` to inspect configured sources and their tool counts.",
    "- The `tools` object is a lazy proxy — `Object.keys(tools)` won't work. Use the discovery helpers instead.",
    "- Pass an object to system tools, e.g. `tools.discover({ query: \"...\" })`, `tools.executor.tools.search({ query: \"...\" })`, `tools.executor.sources.list()`, and `tools.describe.tool({ path })`.",
